...
def train_one_epoch(model: torch.nn.Module,
    ...
    for data_iter_step, (images, signals, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
...
def evaluate(model: torch.nn.Module,
    ...
    for images, signals, targets in metric_logger.log_every(data_loader, 10, header):
        images = images.to(device, non_blocking=True)
        signals = signals.to(device, non_blocking=True)
        targets = targets.to(device, non_blocking=True)
    ...
    metrics = metric_fn.compute() # 이 과정에서 batch마다 누적된 metric이 compute()에 의해 평균화됨 
